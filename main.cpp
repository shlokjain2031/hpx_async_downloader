...
#include </Users/shlokjain/CLionProjects//hpx_async_downloader/benchmarks/include/sequential_downloader.hpp>

#include "benchmarks/include/benchmarks.hpp"

#define URL_PATH "/Users/shlokjain/CLionProjects/hpx_async_downloader/urls.txt"

double parallel_downloader(std::size_t batch_size,
    std::atomic<std::size_t>& total_parallel_bytes_downloaded, std::atomic<std::size_t>& num_of_urls) {
    ...
    const auto read_urls_handle = read_urls_to_queue(URL_PATH, queue, batch_size, num_of_urls);
...
void print_pretty(std::size_t batch_size, std::size_t num_of_urls, unsigned int num_threads, double time_taken, double speedup,
...
void print_benchmarks_csv(std::size_t batch_size, std::size_t num_of_urls, unsigned int num_threads, double time_taken, double speedup,
...
int my_hpx_main(const hpx::program_options::variables_map& vm) {
    ...
    const std::size_t batch_size = vm["batch_size"].as<int>();

    std::atomic<std::size_t> total_parallel_bytes_downloaded = 0;
    std::atomic<std::size_t> total_sequential_bytes_downloaded = 0;

    std::atomic<std::size_t> num_of_urls = 0;

    const double parallel_elapsed = parallel_downloader(batch_size, total_parallel_bytes_downloaded, num_of_urls);
    const double sequential_elapsed = sequential_downloader(total_sequential_bytes_downloaded, num_of_urls);
    ...
    const double throughput_parallel = calculate_throughput(total_parallel_bytes_downloaded, parallel_elapsed);
    const double throughput_sequential = calculate_throughput(total_sequential_bytes_downloaded, parallel_elapsed);
    ...
    if (benchmarking) {
        print_benchmarks_csv(batch_size, num_of_urls.load(), num_of_threads, parallel_elapsed,
                        speedup, efficiency, throughput_parallel, latency_per_task_parallel, throughput_sequential, latency_per_task_sequential);
    }
    else {
        print_pretty(batch_size, num_of_urls.load(), num_of_threads, parallel_elapsed,
...
int main(int argc, char* argv[]) {
    ...
    cmdline.add_options()
            ...
            ("batch_size", hpx::program_options::value<std::size_t>()->default_value(5), "Batch size");
    // clang-format on
